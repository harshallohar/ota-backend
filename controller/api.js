...
exports.getUpdate = asyncHandler(async (req, res, next) => {
  ...
  if (!batch) {
    throw new ErrorResponse(`batch with name: ${req.query.batchName} not found`, 403)
  }
  if (batch.picIds.includes(req.query.picId)) {
    const binDocument = await Bin.find({ batch: batch._id }).sort({ addedDate: -1 }).limit(1)
    ...
    return next(new ErrorResponse(`pic id : ${req.query.picId} not found`, 403))
  }
})
...
exports.uploadBin = asyncHandler(async (req, res, next) => {
  ...
  if (!batch) {
    throw new ErrorResponse(`batch with id : ${req.body.batchId} doesn't exist`, 400)
  }
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}//${file.name}${ext}`, async err => {
    if (err) {
      throw new ErrorResponse('server error happened, please try again later', 500)
    }
...
exports.addSingleBatch = asyncHandler(async (req, res, next) => {
  ...
  if (!manufacturer) {
    throw new ErrorResponse(`manufacturer with ${req.body.manufacturerId}`, 404)
  } else if (await Batch.findOne({ _id: { $in: manufacturer.batches }, name: req.body.name })) {
    throw new ErrorResponse(`batch with name: ${req.body.name} for a manufacturer already exist`, 404)
  } else {
...
exports.addSingleEsp = asyncHandler(async (req, res, next) => {
  try {
    const manufacturer = await Manufacturer.findById(req.body.manufacturerId)
    if (!manufacturer) {
      throw new Error(`manufacturer with ${req.body.manufacturerId} doesn't exist`)
      ...
      const newEsp = await ESP.create({ name: req.body.name, espId: req.body.espId })
...
exports.addSinglePic = asyncHandler(async (req, res, next) => 
{
  try {
    ...
    if (!batch) {
      ...
      throw new Error(`pic with name: ${req.body.name} already exist in the batch`)
    } else {
